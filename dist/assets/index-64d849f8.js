var S=Object.defineProperty;var V=(d,t,s)=>t in d?S(d,t,{enumerable:!0,configurable:!0,writable:!0,value:s}):d[t]=s;var x=(d,t,s)=>(V(d,typeof t!="symbol"?t+"":t,s),s);(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))e(i);new MutationObserver(i=>{for(const n of i)if(n.type==="childList")for(const r of n.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&e(r)}).observe(document,{childList:!0,subtree:!0});function s(i){const n={};return i.integrity&&(n.integrity=i.integrity),i.referrerPolicy&&(n.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?n.credentials="include":i.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function e(i){if(i.ep)return;i.ep=!0;const n=s(i);fetch(i.href,n)}})();const P=`
    .widget__container * {
...
`).map(a=>a.replace(/^data: /,"").trim()).filter(a=>a!==""&&a!=="[DONE]").map(a=>JSON.parse(a));console.log("parsedLines",b);const E={message:t,direction:"outgoing",sender:"user"};for(const a of b){const{choices:f}=a,{delta:L}=f[0],{content:m}=L;if(console.log("content ltst",m),m){let h="";h+=m,this.currentOutput+=h,console.log("contscontscontsconts",h),e.innerText=this.currentOutput;const W={message:h,sentTime:"just now",sender:"Prognosis"};setTimeout(()=>{this.displayArea.scrollTop=this.displayArea.scrollHeight},100)}}}}catch(l){o.aborted?(console.log("Request aborted!"),this.loaderWrapper.remove()):(console.log("Error occured while generating."),console.log("Error ",l),this.loaderWrapper.remove())}finally{this.chatStopBtn.style.display="none",this.submitBtn.style.display="flex",this.controller=null,this.inputBox.value="",this.inputValue="",this.loaderWrapper.remove()}}else alert("Please enter a message")}generateQuestion(t){t.forEach(s=>{console.log("element.title",s.title);const e=document.createElement("div");e.classList.add("prognosis_pre_question_block"),e.setAttribute("id",s.title),e.innerText=s.title,e.addEventListener("click",i=>{this.inputValue=i.currentTarget.id,this.processMessageToChatGPT(),this.questionsWrapper.classList.add("hide_section")}),this.questionsWrapper.appendChild(e)})}askQuestion(t){console.log("question",t.currentTarget.id),this.inputValue=t.currentTarget.id,this.processMessageToChatGPT()}}customElements.define("explain-component",st,{extends:"div"});class it extends HTMLDivElement{constructor(){super(),this.theme=JSON.parse(localStorage.getItem("prognosisTheme")),console.log("this.options",this.theme.primaryColor),this.currentOutput="",this.inputValue="";const t=`
            align-items: center;
            border: 1px solid lightgray;
            border-radius: 15px;
            display: flex;
        ...
        <div class="aside-wrap" id="prognosis_main_nav_wrapper">
            ...
            <div class="prognosis_menu_collapse_btn_holder">
              <button class="prognosis_menu_collapse_btn expanded" id="prognosis_menu_collapse_btn">
                <span class="collapse_arrow_left">${J}</span>
                <span class="collapse_arrow_right">${$}</span>
              </button>
            </div>
            <ul class="prognosis-sidebar-menu-list">
              <li>
                <div class="prognosis-menu-title active" id="prognosis_e_chat">${F} <span class="prognosis-menu-text">New Chat</span></div>
              ...
              <!--<li>
                <div class="prognosis-menu-title">${R} <span class="prognosis-menu-text">Smart Tasks</span></div>
                <ul class="prognosis-menu-children">
