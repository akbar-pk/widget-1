var W=Object.defineProperty;var H=(l,t,e)=>t in l?W(l,t,{enumerable:!0,configurable:!0,writable:!0,value:e}):l[t]=e;var g=(l,t,e)=>(H(l,typeof t!="symbol"?t+"":t,e),e);(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))s(i);new MutationObserver(i=>{for(const n of i)if(n.type==="childList")for(const r of n.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&s(r)}).observe(document,{childList:!0,subtree:!0});function e(i){const n={};return i.integrity&&(n.integrity=i.integrity),i.referrerPolicy&&(n.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?n.credentials="include":i.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function s(i){if(i.ep)return;i.ep=!0;const n=e(i);fetch(i.href,n)}})();const k=`
    .widget__container * {
...
`).map(o=>o.replace(/^data: /,"").trim()).filter(o=>o!==""&&o!=="[DONE]").map(o=>JSON.parse(o));console.log("parsedLines",u);const T={message:t,direction:"outgoing",sender:"user"};for(const o of u){const{choices:x}=o,{delta:y}=x[0],{content:h}=y;if(console.log("content ltst",h),h){let d="";d+=h,this.currentOutput+=d,console.log("contscontscontsconts",d),s.innerText=this.currentOutput;const E={message:d,sentTime:"just now",sender:"Prognosis"};setTimeout(()=>{this.displayArea.scrollTop=this.displayArea.scrollHeight},100)}}}}catch(c){a.aborted?(console.log("Request aborted!"),this.loaderWrapper.remove()):(console.log("Error occured while generating."),console.log("Error ",c),this.loaderWrapper.remove())}finally{this.chatStopBtn.style.display="none",this.submitBtn.style.display="flex",this.controller=null,this.inputBox.value="",this.inputValue="",this.loaderWrapper.remove()}}else alert("Please enter a message")}generateQuestion(t){t.forEach(e=>{console.log("element.title",e.title);const s=document.createElement("div");s.classList.add("prognosis_pre_question_block"),s.setAttribute("id",e.title),s.innerText=e.title,s.addEventListener("click",i=>{this.inputValue=i.currentTarget.id,this.processMessageToChatGPT(),this.questionsWrapper.classList.add("hide_section")}),this.questionsWrapper.appendChild(s)})}generateChatHomeQuestion(t){t.forEach(e=>{const s=document.createElement("h4");s.classList.add("prognosis_chat_question_item"),s.setAttribute("id",e.question);const i=document.createElement("span");switch(i.classList.add("prognosis_chat_question_icon"),e.icon){case"heart":i.innerHTML=V;break;case"bloodPressure":i.innerHTML=O;break;case"brain":i.innerHTML=R;break;case"food":i.innerHTML=P;break}s.appendChild(i);const n=document.createElement("span");n.classList.add("prognosis_question_text_holder"),n.innerText=e.question,s.appendChild(n),s.addEventListener("click",r=>{this.chatHomeWrapper.classList.add("hide_page"),this.inputValue=r.currentTarget.id,this.processMessageToChatGPT(),this.questionsWrapper.classList.add("hide_section")}),this.chatLetListHolder.appendChild(s)})}askQuestion(t){console.log("question",t.currentTarget.id),this.inputValue=t.currentTarget.id,this.processMessageToChatGPT()}}customElements.define("consultation-component",z,{extends:"div"});class F extends HTMLDivElement{constructor(){super(),this.questions=[{title:"What is Atrial Fibrillation?"},{title:"What is Stroke?"},{title:"What is HF Stage B?"},{title:"What is Hypertension?"},{title:"What is what is Anaemia?"}],this.questionsWrapper=document.createElement("div"),this.questionsWrapper.classList.add("prognosis_pre_qa_wrapper"),this.createQuestion(),this.generateQuestion(this.questions)}createQuestion(){this.appendChild(this.questionsWrapper)}generateQuestion(t){t.forEach(e=>{console.log("element.title",e.title);const s=document.createElement("div");s.classList.add("prognosis_pre_question_block"),s.innerText=e.title,s.addEventListener("click",this.askQuestion(e)),this.questionsWrapper.appendChild(s)})}askQuestion(t){console.log("question",t)}}customElements.define("pre-defined-questions",F,{extends:"div"});class G extends HTMLDivElement{constructor(){super(),this.theme=JSON.parse(localStorage.getItem("prognosisTheme")),console.log("this.options",this.theme.primaryColor),this.currentOutput="",this.inputValue="",this.questions=[{title:"ICD-10-CM"},{title:"CPT"},{title:"HCPCS"},{title:"DRG"},{title:"SNOMED CT"},{title:"LOINC"}];const t=`
        ...
        `;this.submitBtn=document.createElement("button"),this.submitBtn.classList.add("prognosis__consult_submitBtn"),this.submitBtn.innerHTML=f,this.submitBtn.setAttribute("style",s),this.submitBtn.addEventListener("click",()=>{this.processMessageToChatGPT()}),this.chatStopBtn=document.createElement("button"),this.chatStopBtn.classList.add("prognosis_chat_stop_btn"),this.chatStopBtn.innerHTML=w,this.chatStopBtn.setAttribute("style",s),this.chatStopBtn.title="Stop",this.chatStopBtn.style.display="none",this.chatStopBtn.addEventListener("click",()=>{this.controller&&(this.controller.abort(),this.controller=null,this.clearChatBtn.style.display="flex")});const i=`
                background-color: lightgray;
                color: gray;
                fill: gray;
...
`).map(o=>o.replace(/^data: /,"").trim()).filter(o=>o!==""&&o!=="[DONE]").map(o=>JSON.parse(o));console.log("parsedLines",u);const T={message:t,direction:"outgoing",sender:"user"};for(const o of u){const{choices:x}=o,{delta:y}=x[0],{content:h}=y;if(console.log("content ltst",h),h){let d="";d+=h,this.currentOutput+=d,console.log("contscontscontsconts",d),s.innerText=this.currentOutput;const E={message:d,sentTime:"just now",sender:"Prognosis"};setTimeout(()=>{this.displayArea.scrollTop=this.displayArea.scrollHeight},100)}}}}catch(c){a.aborted?console.log("Request aborted!"):(console.log("Error occured while generating."),console.log("Error ",c)),this.loaderWrapper.remove()}finally{this.chatStopBtn.style.display="none",this.submitBtn.style.display="flex",this.controller=null,this.inputBox.value="",this.inputValue="",this.loaderWrapper.remove()}}else alert("Please enter a message")}generateQuestion(t){t.forEach(e=>{console.log("element.title",e.title);const s=document.createElement("div");s.classList.add("prognosis_pre_question_block"),s.setAttribute("id",e.title),s.innerText=e.title,s.addEventListener("click",i=>{this.inputValue=i.currentTarget.id,this.processMessageToChatGPT(),this.questionsWrapper.classList.add("hide_section")}),this.questionsWrapper.appendChild(s)})}askQuestion(t){console.log("question",t.currentTarget.id),this.inputValue=t.currentTarget.id,this.processMessageToChatGPT()}}customElements.define("medical-coding",G,{extends:"div"});class j extends HTMLDivElement{constructor(){super(),this.theme=JSON.parse(localStorage.getItem("prognosisTheme")),console.log("this.options",this.theme.primaryColor),this.currentOutput="",this.inputValue="";const t=`
            align-items: center;
            border: 1px solid lightgray;
            border-radius: 15px;
            display: flex;
...
`).map(o=>o.replace(/^data: /,"").trim()).filter(o=>o!==""&&o!=="[DONE]").map(o=>JSON.parse(o));console.log("parsedLines",u);const T={message:t,direction:"outgoing",sender:"user"};for(const o of u){const{choices:x}=o,{delta:y}=x[0],{content:h}=y;if(console.log("content ltst",h),h){let d="";d+=h,this.currentOutput+=d,console.log("contscontscontsconts",d),s.innerText=this.currentOutput;const E={message:d,sentTime:"just now",sender:"Prognosis"};setTimeout(()=>{this.displayArea.scrollTop=this.displayArea.scrollHeight},100)}}}}catch(c){a.aborted?(console.log("Request aborted!"),this.loaderWrapper.remove()):(console.log("Error occured while generating."),console.log("Error ",c),this.loaderWrapper.remove())}finally{this.chatStopBtn.style.display="none",this.submitBtn.style.display="flex",this.controller=null,this.inputBox.value="",this.inputValue="",this.loaderWrapper.remove()}}else alert("Please enter a message")}generateQuestion(t){t.forEach(e=>{console.log("element.title",e.title);const s=document.createElement("div");s.classList.add("prognosis_pre_question_block"),s.setAttribute("id",e.title),s.innerText=e.title,s.addEventListener("click",i=>{this.inputValue=i.currentTarget.id,this.processMessageToChatGPT(),this.questionsWrapper.classList.add("hide_section")}),this.questionsWrapper.appendChild(s)})}askQuestion(t){console.log("question",t.currentTarget.id),this.inputValue=t.currentTarget.id,this.processMessageToChatGPT()}}customElements.define("explain-component",j,{extends:"div"});class Q extends HTMLDivElement{constructor(){super(),this.theme=JSON.parse(localStorage.getItem("prognosisTheme")),console.log("this.options",this.theme.primaryColor),this.currentOutput="",this.inputValue="";const t=`
            align-items: center;
            border: 1px solid lightgray;
            border-radius: 15px;
            display: flex;
        ...
        <div class="aside-wrap" id="prognosis_main_nav_wrapper">
            ...
            <div class="prognosis_menu_collapse_btn_holder">
              <button class="prognosis_menu_collapse_btn collapsed" id="prognosis_menu_collapse_btn">
                <span class="collapse_arrow_left">${Z}</span>
                <span class="collapse_arrow_right">${N}</span>
              </button>
            </div>
            <ul class="prognosis-sidebar-menu-list">
              <li>
                <div class="prognosis-menu-title active" id="prognosis_e_chat">${S} <span class="prognosis-menu-text">Chat</span></div>
